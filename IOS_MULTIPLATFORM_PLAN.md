...
|---|---|---|---|
| **4a** | Password hashing (`PasswordHasher.kt`: `javax.crypto`/`java.security`, JVM-only) | ✅ Done — 2026-07-07 | Smallest scope, no UI/DI coupling, hard blocker for login — do first |
| 4b | Secure storage (`SessionStore.kt`: Android Keystore/`EncryptedSharedPreferences` → iOS Keychain) | ✅ Done — 2026-07-07 | Small, but needs a new cinterop pattern (`platform.Security`); do right after 4a while crypto/cinterop context is fresh |
| 4c | Background sync scheduling (`SyncWorker.kt`: WorkManager → iOS `BGTaskScheduler`) | ✅ Done (compile-verified only — see below) — 2026-07-07 | No UI dependency, but real design surface (Info.plist `BGTaskSchedulerPermittedIdentifiers`/`UIBackgroundModes`) — apply the Phase 3 `info.properties` lesson from the start instead of rediscovering it |
| 4d | Notifications (`CustomsNotificationManager.kt`: `NotificationManager` → `UNUserNotificationCenter`) | ✅ Done (compile-verified only — see below) — 2026-07-07 | Note: `postSlaNotification` is currently dead code (unused, likely reserved for the not-yet-built `SlaCheckerWorker` from the original 7-phase workflow plan) — port the API shape for parity but don't over-invest in testing an unused path |
...
### Phase 4a — Password hashing ✅ Done — 2026-07-07

`PasswordHasher` moved to `commonMain` as an `expect object`. Android actual keeps the existing `javax.crypto` `PBKDF2WithHmacSHA256` implementation unchanged (100k iterations, 256-bit key, 16-byte salt) — zero behavior change, every existing stored password hash remains valid. iOS actual uses Apple's CommonCrypto `CCKeyDerivationPBKDF` with matching parameters — PBKDF2 is a deterministic standard, so identical algorithm/iteration-count/salt/key-length parameters produce byte-identical output on both platforms. Chose native platform crypto over a third-party KMP crypto library (considered `dev.whyoleg:cryptography-kotlin`, but its own GitHub issues showed its PBKDF2 API under-documented with real usage friction) — password hashing is exactly the security-sensitive surface where a decade-old native API beats a younger convenience library.

Added a `commonTest` round-trip test plus an independent known-answer test (a fixed salt/hash pair computed via Python's `hashlib.pbkdf2_hmac`, not derived from either platform's own actual) to guard against both actuals silently drifting from the PBKDF2 standard in a way that only happens to agree with each other. Wired a new `:app:iosSimulatorArm64Test` CI step, run before the Xcode/app-build steps, so the iOS actual is actually *executed* on the iOS Simulator, not just compiled.

**Five CI round-trips to get CommonCrypto interop working — the debugging path, since each wrong turn is a real lesson for future cinterop work in this project:**

1. **First attempt** (hand-written `.def` with `headers = CommonCrypto/CommonKeyDerivation.h`, no `package` line, declared via `cinterops {}` in `iosMain`): failed with `Unresolved reference 'CommonCrypto'`. Assumed cause: cinterop commonization needed for a custom binding to be visible from the shared `iosMain` source set.
2. **Second attempt**: added `kotlin.mpp.enableCInteropCommonization=true`. Identical failure — the log showed only the single target being built (`:app:iosSimulatorArm64Test` never builds `iosArm64`'s klib too), so commonization's cross-target merge never had both halves to work with.
3. **Third attempt**: sidestepped commonization by pushing the CommonCrypto call into duplicated per-target leaf source sets (`iosArm64Main`/`iosSimulatorArm64Main`). Still `Unresolved reference 'CommonCrypto'` — proved the problem was never about source-set visibility at all.
4. **Fourth attempt**: found the actual bug — the `.def` file never declared a `package` line, so the binding never generated under `platform.CommonCrypto` in any configuration. Added `package = platform.CommonCrypto`. Progress: the *package* now resolved, but its specific symbols (`CCKeyDerivationPBKDF`, `kCCPBKDF2`, `kCCPRFHmacAlgSHA256`) still didn't — a single-header `headers = ...` include doesn't reliably expose everything a proper Clang-module import does.
5. **Fifth attempt (the real fix)**: checked Kotlin/Native's own source (`kotlin-native/platformLibs/src/platform/ios/CommonCrypto.def`) instead of guessing further. **CommonCrypto already ships as one of Kotlin/Native's own bundled platform libraries** — no custom cinterop needed at all, exactly like `platform.Foundation`/`platform.Security` used elsewhere in this project. Its real package is **`platform.CoreCrypto`** (a historical naming quirk, not `platform.CommonCrypto`), imported via `modules = CommonCrypto` (a proper Clang module, not a single header). Deleted the entire custom cinterop setup — `.def` file, Gradle `cinterops{}` wiring, the two leaf source sets — and imported `platform.CoreCrypto` directly. This got past the unresolved-reference class of error entirely; the one remaining issue was a precise, compiler-stated type mismatch (the bundled binding declares `password` as a Kotlin `String`, auto-bridging the C `const char *`, not a raw pointer like `salt`/`derivedKey`) — fixed in one line.

**Lesson for future cinterop work in this project: check Kotlin/Native's own bundled platform library defs on GitHub (`kotlin-native/platformLibs/src/platform/<target>/`) before writing a custom cinterop for any Apple system framework.** It may already be there, possibly under a non-obvious package name.

**Verified on iOS CI ([run 28874023056](https://github.com/balor195/rms-customs-tracker/actions/runs/28874023056)):** all 4 `PasswordHasherTest` cases genuinely executed and passed on the iOS Simulator via Kotlin/Native (not just compiled) — critically including the independent known-vector test, proving CommonCrypto's PBKDF2 output is byte-identical to the Python reference (and thus to Android's `javax.crypto`). The existing app-launch flow is unaffected: built `Info.plist` still has `CADisableMinimumFrameDurationOnPhone`, console/crash logs are clean, and the screenshot shows crisp, correctly-rendered UI (the low-contrast rendering noted as an open observation after Phase 3 did not recur here).

### Phase 4b — Secure storage ✅ Done — 2026-07-07

`SessionStore` moved to `commonMain` as an `expect class`, taking a new `expect class PlatformContext` wrapper instead of Android's `Context` directly (`Context` is abstract, which conflicts with an expect class's implicit `final` modality if aliased directly via `actual typealias` — a thin wrapper class sidesteps this; caught locally before wasting a CI round-trip, along with two more expect/actual mismatches: constructor parameter names must match exactly between expect and actual, and shadowing a constructor parameter with a same-named property caused self-referential type resolution). Android's actual keeps the existing `EncryptedSharedPreferences` logic verbatim. iOS's actual is Keychain Services-backed (`NSMutableDictionary` + `SecItemAdd`/`SecItemCopyMatching`/`SecItemDelete`), storing the session as one colon-joined string, mirroring `PasswordHasher`'s own convention from 4a.

**Five CI rounds, each narrowing the problem, then an external (billing) blocker, then final confirmation:**
1. `NSMutableDictionary.setObject` rejected the raw `kSec*` constants (`CPointer<__CFString>?`) passed where `NSCopyingProtocol` was expected — needed `.dataUsingEncoding`/`NSString.create` too, both of which turned out to have uncertain Kotlin-generated signatures; switched to raw byte-buffer `NSData` creation/extraction instead, avoiding NSString-based conversion.
2. Missing `import platform.Foundation.create` — Kotlin/Native's `.create(...)` factory-call syntax needs its own explicit import, separate from importing the class itself.
3. **Runtime** (not compile-time) `ClassCastException`: `class kotlinx.cinterop.CPointer cannot be cast to class platform.Foundation.NSString`. An unchecked `as NSString` cast on a `kSec*` constant compiles but doesn't perform genuine toll-free bridging — needed `CFBridgingRelease(constant) as NSString` instead.
4. Same issue, opposite direction: `class kotlin.native.internal.NSDictionaryAsKMap cannot be cast to class kotlinx.cinterop.CPointer` — a Kotlin-constructed `NSMutableDictionary` needs `CFBridgingRetain(dict) as CFDictionaryRef` to bridge to a real CF pointer, not a raw cast (and since this transfers a +1 reference, each query now gets an explicit `CFRelease` after its Security API call).
5. Down to one real failure: `save()` then `load()` returned `null`. Root cause is **not a code bug** — it's a documented Kotlin/Native limitation, [JetBrains YouTrack KT-61470](https://youtrack.jetbrains.com/issue/KT-61470) ("Keychain operations fail in iOS tests due to missing entitlements"): `./gradlew iosSimulatorArm64Test` runs a bare, unsigned test binary directly via `simctl`, never wrapped in a real app bundle with an entitlements plist, and `SecItemAdd` requires `keychain-access-groups` entitlements at runtime to actually persist an item. `clear()`/`load()`-when-empty both passed correctly (no entitlements needed for a no-op/not-found query), confirming this diagnosis. Marked the round-trip test `@Ignore` with an explanation, rather than chasing test-harness entitlements further — the same practical gap Android's own `SessionStore` already has (no unit test, since `EncryptedSharedPreferences`/Keystore access also needs a real device/emulator, not a plain JVM test).

**External blocker (2026-07-07):** the next CI push failed to even start — GitHub Actions reported a billing/spending-limit error unrelated to any code. Root cause: this project's free-tier GitHub Actions minutes (2,000/month) were exhausted by this migration's own heavy iOS CI usage — macOS runners consume that quota at a 10x multiplier, and this session alone ran the workflow ~20 times. Resolved by making the `rms-customs-tracker` repo public (public repos get unlimited free Actions minutes, including macOS) — a deliberate, disclosed tradeoff given this project's government-adjacent context, decided by the user after the alternatives (wait for monthly reset, or add a paid spending limit) were laid out.

**Verified on iOS CI ([run 28893979308](https://github.com/balor195/rms-customs-tracker/actions/runs/28893979308)):** `SessionStoreTest` shows 3 tests, 1 skipped (the `@Ignore`d round-trip, per the entitlements limitation above), 0 failures — `clear()` and `load()`-when-empty both genuinely pass on the iOS Simulator. `PasswordHasherTest` still 4/4 (no regression). App-launch flow unaffected: `Info.plist` still has `CADisableMinimumFrameDurationOnPhone`, console/crash logs clean, screenshot shows crisp UI.

### Phase 4c — Background sync scheduling ✅ Done (compile-verified) — 2026-07-07

`BackgroundSyncScheduler` added to `commonMain` as an `expect class`, reusing the `PlatformContext` abstraction from 4b. Android actual moves the existing `WorkManager` periodic-enqueue logic out of `CustomsApp`'s private `scheduleSyncWorker()` unchanged in effect (same 15-minute interval, network constraint, `KEEP` policy); `SyncWorker.kt` itself is untouched. iOS actual uses `platform.BackgroundTasks` (confirmed to be one of Kotlin/Native's own bundled platform libraries before writing any code — applying 4a's lesson directly instead of rediscovering it) to register a `BGAppRefreshTask` handler that calls `syncRepository.sync()` in a coroutine and re-submits the next request each time (BGTask requests are one-shot, unlike Android's periodic request). Added `UIBackgroundModes`/`BGTaskSchedulerPermittedIdentifiers` to `project.yml`'s `info.properties` block, applying the Phase 3 lesson from the start.

**Set the verification bar explicitly before writing any code, learning directly from 4b instead of rediscovering the same limitation:** `BGTaskScheduler.register()` needs `BGTaskSchedulerPermittedIdentifiers` in a *real* app's Info.plist — a bare `iosSimulatorArm64Test` binary has none, the same gap that made 4b's Keychain round-trip untestable. No `iosTest` behavioral test was written for `scheduleSync()`; this sub-phase's CI verification is compilation only, real behavior deferred to Phase 5+ once wired into the live app.

**Four CI rounds to get the BGTaskScheduler API right, each error narrower than the last:**
1. `dateWithTimeIntervalSinceNow` unresolved, plus a missing `@OptIn(ExperimentalForeignApi::class)`, plus `submitTaskRequest` needing an explicit `error` parameter (doesn't auto-convert to a throwing function like some ObjC NSError\*\*-returning methods do) — fixed the latter two; for the first, guessed a different NSDate convenience method name (`dateByAddingTimeInterval`).
2. That guess also failed to resolve.
3. Rather than guess a third convenience-method name, switched to `NSDate`'s designated initializer pattern — reliable because Kotlin/Native cinterop always maps ObjC `-init...` methods to a Kotlin constructor. Guessed the parameter name `timeIntervalSince1970`.
4. Wrong guess again, but the compiler error listed the exact three real constructors available (`()`, `(timeIntervalSinceReferenceDate: Double)`, `(coder: NSCoder)`) — `timeIntervalSince1970` isn't one of them. Used `timeIntervalSinceReferenceDate` instead, converting from the Unix epoch via the fixed, well-known 978307200-second offset between NSDate's reference date (2001-01-01) and the Unix epoch (1970-01-01).

**Verified on iOS CI ([run 28893979308](https://github.com/balor195/rms-customs-tracker/actions/runs/28893979308)) — compilation and existing-test/launch regression check only, per the verification bar set above:** `:app:iosSimulatorArm64Test` compiled the new iOS actual successfully; `PasswordHasherTest` (4/4) and `SessionStoreTest` (2/3, 1 known-skip) both unaffected; app-launch flow (Info.plist, console/crash logs, screenshot) unaffected by the `project.yml` change.

### Phase 4d — Notifications ✅ Done (compile-verified) — 2026-07-07

`CustomsNotificationManager` moved to `commonMain` as an `expect class`, reusing `PlatformContext`. Android actual is behaviorally identical to before (same 3 channels, same `NotificationCompat`/`PendingIntent` logic); `postSlaNotification` remains dead code on Android (confirmed no call sites anywhere), ported for API parity only, per the roadmap's own note not to over-invest there. iOS actual uses `platform.UserNotifications` (confirmed bundled before writing any code, same practice as 4c) — `createChannels()` maps to requesting notification authorization (no channel concept on iOS); both post methods build a `UNMutableNotificationContent` + immediate `UNNotificationRequest`, stashing `transaction_id` in `userInfo` for Phase 5's eventual tap-to-navigate handling (no `PendingIntent` equivalent wired yet). Escalated SLA notifications get `.timeSensitive` interruption level; everything else gets `.active`. No `project.yml` changes needed — unlike Keychain/`BGTaskScheduler`, local notifications need no special Info.plist entries.

**Verification bar set explicitly up front, same practice as 4c:** a bare test binary isn't a real registered app for the notification system to authorize against — no `iosTest` behavioral test written; compilation is the bar.

**Two CI rounds, both errors matching uncertainties already flagged in the plan before writing any code:**
1. `UNNotificationInterruptionLevelActive`/`TimeSensitive` don't exist as top-level constants — `UNNotificationInterruptionLevel` is a proper Kotlin enum class, entries accessed as `UNNotificationInterruptionLevel.UNNotificationInterruptionLevelActive`. Separately, `UNNotificationRequest`'s compiler error listed only two real constructors (`()`, `(coder: NSCoder)`) — `identifier:content:trigger:` is an ObjC class factory method (`+requestWithIdentifier:content:trigger:`), not a designated initializer, so it binds as `UNNotificationRequest.requestWithIdentifier(...)` instead of a constructor call.
2. Both fixes compiled clean on the first retry.

**Verified on iOS CI ([run 28902987957](https://github.com/balor195/rms-customs-tracker/actions/runs/28902987957)) — compilation and regression check only:** `:app:iosSimulatorArm64Test` compiled the new iOS actual successfully; `PasswordHasherTest` (4/4) and `SessionStoreTest` (2/3, 1 known-skip) unaffected; app-launch flow unaffected (an unrelated `TypeToSiriWidgetExtension` system-component crash report appeared in this run's diagnostics — a known iOS Simulator quirk, not related to this app's process, which showed zero crashes/exceptions of its own).
