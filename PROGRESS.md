...
**Phase 4b done (2026-07-07)** — secure storage: `SessionStore` moved to `commonMain` as an `expect class` (via a new `PlatformContext` wrapper, since `android.content.Context` is abstract and can't be `actual typealias`'d directly against an expect class). Android actual unchanged (`EncryptedSharedPreferences`); iOS actual is Keychain Services-backed. Five CI rounds fixed real toll-free-bridging bugs (`NSMutableDictionary`/`CFDictionaryRef`/`NSString` casts all need explicit `CFBridgingRetain`/`CFBridgingRelease`, not raw `as` casts) down to one genuine, documented Kotlin/Native limitation (JetBrains `KT-61470`: bare test binaries lack Keychain entitlements) — marked that one test `@Ignore`'d rather than chased further. Hit an unrelated **external blocker** mid-phase: GitHub Actions billing exhausted (macOS CI runners burn the free-tier quota at 10x), resolved by making the repo public (a deliberate, disclosed call given this project's context). Verified for real once unblocked: `clear()`/`load()`-when-empty pass on the iOS Simulator, `PasswordHasherTest` unaffected, app-launch flow unaffected.
